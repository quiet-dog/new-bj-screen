...
stompClient.onConnect = frame => {
    stompClient.subscribe("/topic/data", greeting => {
        ...
        for (let i in equipmentData.value[currentFloor]) {
            console.log(equipmentData.value[currentFloor]);
            for (let j in equipmentData.value[currentFloor][i].thresholdList) {
                console.log(equipmentData.value[currentFloor][i].thresholdList[j].thresholdId, data.content.equipmentInfo.thresholdId);
...
const parseMapFloor = (data) => {
    ...
    for (let i in data) {
        let floor = data[i].installationLocation[2]
        if (!["1", "2", "3", "4"].includes(floor)) continue;
        result[floor].push(data[i])
    }
    return result
}
const currentFloorData = computed(() => {
    let currentFloor = floorStore.currentFloor.toString();
    let currentFloorData = equipmentData.value[currentFloor[0]];
...
onMounted(async () => {
    ...
    equipmentData.value = parseMapFloor(await getEquipmentData())
